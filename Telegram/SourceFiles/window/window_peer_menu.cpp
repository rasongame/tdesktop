...
QPointer<Ui::BoxContent> ShowOldForwardMessagesBox(
	...
	const auto showMenu = [=](not_null<Ui::RpWidget*> parent) {
		...
		SendMenu::FillSendMenu(
			...
			SendMenu::DefaultCallback(show, crl::guard(parent, state->submit)));
	...
	field->submits(
	) | rpl::start_with_next([=] { state->submit({}); }, field->lifetime());
...
QPointer<Ui::BoxContent> ShowForwardMessagesBox(
	...
	state->submit = [=](Api::SendOptions options) {
		...
		/// vvv from Official Telegram Desktop
		...
		/// ^^^ from Official Telegram Desktop
		send(
			ranges::views::all(
				peers
			) | ranges::views::transform([&](
